...
def main():
    with sqlite3.connect("test.sqlite") as conn:
        ...
        # for car_photo in car_photo_table.get_all():
        #     for car in car_table.get_all():
        #         if car_photo.car_id_fn == car.id:
        #             logging.debug(car.brand)
        #             logging.debug("| brand | model | url |")
        #             logging.debug(f"|{car.brand} | {car.model} | {car_photo.photo_url} |")
        # The same:
        # SELECT car.brand, car.model, car_photo.photo_url
        # FROM cars
        # RIGHT JOIN car_photo ON car_photo.car_id_fn == car.id;
